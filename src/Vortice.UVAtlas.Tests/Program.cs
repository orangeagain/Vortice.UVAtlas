    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var mesh in meshSet) {
                Console.WriteLine("NasaWrap.ComputeUV() Running..." + mesh.Key);
                ...
                Console.WriteLine("Vortice.ComputeUV Running... on " + mesh.Key);
                ...
                if (nasaSuccess == false || vorticeSuccess == false)
                    Console.WriteLine("Faild Mesh:" + mesh .Key + "|nasaSuccess:"+ nasaSuccess+ "|vorticeSuccess:"+ vorticeSuccess);
            }
        }
        public static bool VorticeComputeUV(Vector3[] inputPos, uint[] indices, out int[] outIndices, out int[] outVertexRemap, out Vector2[] outUV)
        {
            ...
            outVertexRemap = null;
            outUV = null;
            if (!VorticeUVAtlasWrap.GenerateAdjacencyAndPointReps_Wrap(indices, inputPos, out uint[] adj))
            {
                Console.WriteLine("Native.GenerateAdjacencyAndPointReps() failure");
                return false;
            }

            if (!VorticeUVAtlasWrap.Create(inputPos, indices, adj, out VorticeUVAtlasWrap.UvResult uvResult))
            {
                Console.WriteLine("Native.UVAtlas.Create() failure");
                return false;
            }
